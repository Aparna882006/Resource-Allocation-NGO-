    ...
    const panelHTML = `
      <div id="globalProfilePanel" class="global-profile-panel">
          ...
          <div class="panel-body">
             <button onclick="window.location.href='volunteer-profile.html'" class="btn-action" style="width:100%; margin-bottom:10px; background:var(--primary-light); color:var(--primary-dark); border:none; justify-content:center;"><i class="fas fa-id-card"></i> View Full Profile</button>
             <hr style="margin: 10px 0; border:none; border-top:1px solid var(--border-color);">
             <button onclick="auth.signOut().then(() => { localStorage.clear(); window.location.href='login.html'; })" class="btn-action" style="width:100%; justify-content:center; background:var(--emergency-red); color:#ffffff; border:none;"><i class="fas fa-sign-out-alt"></i> Logout</button>
          </div>
      </div>
    `;
        ...
        if(user) {
            ...
            try {
                ...
                if(snap.exists) {
                    ...
                    if(roleEl) roleEl.textContent = (d.role || 'user').charAt(0).toUpperCase() + (d.role || 'user').slice(1);
